...
import { SubItemsSelector } from '@/components/menu/sub-items-selector'
...
// Convert add-ons to sub-items format with 3 categories
function convertToSubItems(addOns: AddOn[]) {
  const total = addOns.length
  const perCategory = Math.ceil(total / 3)
  
  return addOns.map((addOn, index) => {
    let category: 'category1' | 'category2' | 'category3' = 'category1'
    if (index < perCategory) {
      category = 'category1'
    } else if (index < perCategory * 2) {
      category = 'category2'
    } else {
      category = 'category3'
    }
    ...
    return {
      ...
      price: addOn.price,
      calorieModifier: addOn.calorieModifier,
      category,
    }
  })
}

export function ProductDetailWithSubItems({ item }: ProductDetailProps) {
  ...
  const subItems = convertToSubItems(item.addOns.filter(a => a.isAvailable))
  ...
  const calculateNutrition = () => {
    ...
    selectedSubItems.forEach(subItem => {
      calories += subItem.calorieModifier
    })
    return {
      ...item.nutrition,
  ...
  const calculatePrice = () => {
    ...
    const subItemsPrice = selectedSubItems.reduce((sum, subItem) => sum + subItem.price, 0)
    return (item.basePrice + sizePrice + addOnsPrice + subItemsPrice) * quantity
  }
  ...
  const handleMealNameConfirm = (name: string) => {
    ...
    // Convert sub-items back to add-ons format
    const subItemsAsAddOns: AddOn[] = selectedSubItems.map(subItem => ({
      id: subItem.id,
      name: subItem.name,
      description: subItem.description,
      price: subItem.price,
      calorieModifier: subItem.calorieModifier,
    ...
    addItem(item, {
      ...
      selectedSubItems: subItemsAsAddOns,
    ...
    <>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          ...
          {/* Sub-Items (3 Categories) */}
          {subItems.length > 0 && (
            <div>
              <Label className="text-base font-semibold mb-3 block">Add Sub-Items</Label>
              <p className="text-sm text-muted-foreground mb-4">
                Select items from any or all 3 categories. You can choose up to 3 items per category.
              </p>
              <SubItemsSelector
                subItems={subItems}
                selectedItems={selectedSubItems}
                onSelectionChange={setSelectedSubItems}
                maxPerCategory={3}
              />
            </div>
          )}
